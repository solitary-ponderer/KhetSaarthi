...
def get_user_inputs():
    ...
    try:
        humidity = float(
            input("Enter Air Humidity (%) [e.g., 65.0]        : ")
        )
    except ValueError:
        humidity = 65.0
    ...
    try:
        pressure = float(
            input("Enter Atmospheric Pressure (hPa) [e.g., 1013.25] : ")
        )
    except ValueError:
        pressure = 1013.25
...
def fetch_database_knowledge(region_id):
    """Executes multi-table SQL join across knowledge tables."""
    query = text("""
        SELECT 
...
def run_recommendation_engine():
    ...
    unique_crops["humidity_ok"] = unique_crops.apply(
        lambda r: (pd.notna(r["min_humidity_pct"]) and pd.notna(r["max_humidity_pct"])) and
              (r["min_humidity_pct"] <= float(inputs["humidity"]) <= r["max_humidity_pct"]),
        axis=1,
    )
    ...
    if disaster_override:
        ...
        unique_crops["final_score"] = unique_crops[
            ["temp_ok", "moisture_ok", "humidity_ok", "lux_ok"]
        ].sum(axis=1)
    ...
    print(
        f"🧪 Measured Parameters  : Temp: {inputs['temperature']}°C | Air Humidity: {inputs['humidity']}% | Soil Moisture: {inputs['soil_moisture']}%"
    )
    print(
        f"📊 Atmospheric Pressure : {inputs['pressure']} hPa"
    )
    ...
    for idx, item in enumerate(final_display_list, 1):
        ...
        print(
            f"    • Evaluation Score : {item['final_score']}/4 (Temp: {item['temp_ok']} | Moisture: {item['moisture_ok']} | Humidity: {item['humidity_ok']} | Lux: {item['lux_ok']})"
        )
        print(f"    • Waterlog Safety  : {item['waterlogging_tolerance']} Tolerance")
        print(f"    • Expected Yield   : {item['observed_yield_mt_ha']} MT/Ha")
        print(f"    • Irrigation Plan  : {item['primary_irrigation_method']}")

        fert_info = crop_rows[["fertilizer_type", "fert_dose"]].dropna().drop_duplicates()
        if not fert_info.empty:
            f_row = fert_info.iloc[0]
            print(f"    • Recommended Fert : {f_row['fertilizer_type']} ({f_row['fert_dose']})")

        pest_info = crop_rows[["disease", "pesticide_type"]].dropna().drop_duplicates()
        if not pest_info.empty:
            p_row = pest_info.iloc[0]
            print(f"    • Disease Control   : Target: {p_row['disease']} | Chemical: {p_row['pesticide_type']}")
